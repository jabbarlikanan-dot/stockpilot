...
function card(w){
  ...
  const offers=(w.offers||[]).map(o=>`<div class="ai-offer"><div><b>${esc(o.title)}</b><small>${esc(o.source)} · Təsdiqlənmiş qiymət · Məhsul ${money(o.productPriceAzn)} · Karqo ${money(o.shippingAzn)}</small></div><div class="ai-offer-price"><b>${money(o.totalAzn)}</b><a href="${esc(o.url)}" target="_blank" rel="noopener noreferrer">Mənbəyə bax ↗</a></div></div>`).join('');
...
async function autoScanIfNeeded(){
  const watches=(payload.watches||[]).filter(w=>w.enabled);
  const stale=watches.filter(w=>!w.lastScanAt || Date.now()-new Date(w.lastScanAt).getTime()>6*60*60*1000);
  if(!stale.length) return;
  const key='stockpilotAiAutoScanAt';
  const last=Number(sessionStorage.getItem(key)||0);
  if(Date.now()-last<10*60*1000) return;
  sessionStorage.setItem(key,String(Date.now()));
  setStatus(`${stale.length} məhsul avtomatik yoxlanır…`);
  try{
    const r=await api('/api/ai-purchases/scan-all',{method:'POST',body:'{}'});
    const d=await r.json().catch(()=>({}));
    if(!r.ok) throw new Error(d.error||'Avtomatik yoxlama alınmadı');
    await refresh();
    setStatus(`${d.scanned||0} məhsul avtomatik yoxlanıldı. Qiymətlər yalnız məhsul səhifəsində təsdiqlənəndə göstərilir.`);
  }catch(e){setStatus(e.message||'Avtomatik yoxlama alınmadı','error')}
}
