...
if (!/^[a-z0-9_.-]{3,30}$/i.test(username)) {
  console.error('İstifadə: node auth-diagnose.mjs username');
  process.exit(1);
}

const escapedUsername = username.replaceAll("'", "''");
const sqlPath = resolve('auth-diagnose-query.sql');
const jsonPath = resolve('auth-diagnose-result.json');
writeFileSync(sqlPath, `SELECT username,password_hash,salt,password_iterations FROM users WHERE username='${escapedUsername}' COLLATE NOCASE LIMIT 1;\n`, 'utf8');

try {
  const comspec = process.env.ComSpec || process.env.COMSPEC || 'cmd.exe';
  const command = `npx.cmd wrangler d1 execute stockpilot-db --remote --file "${sqlPath}" --json > "${jsonPath}"`;
  execFileSync(comspec, ['/d','/s','/c', command], { encoding:'utf8', stdio:['ignore','pipe','pipe'] });
} catch (e) {
...
let raw='';
try { raw = readFileSync(jsonPath, 'utf8'); }
catch { console.error('Wrangler JSON nəticə faylı yaranmadı.'); process.exit(1); }
finally {
  if (existsSync(sqlPath)) unlinkSync(sqlPath);
  if (existsSync(jsonPath)) unlinkSync(jsonPath);
}
...
try { payload = JSON.parse(raw); }
  ...
  if (start < 0 || end < start) {
    console.error('Wrangler JSON cavabı parse olunmadı.');
...
const rows = Array.isArray(payload)
  ? (payload[0]?.results || payload[0]?.result || [])
...
if(!/^\d{4}$/.test(pin)){
  console.error('PIN 4 rəqəm olmalıdır.');
  process.exit(1);
}
...
console.log(match
  ? 'NƏTİCƏ: D1-dəki hash bu PIN-lə uyğundur. Problem Worker login axınındadır.'
  : 'NƏTİCƏ: D1-dəki hash bu PIN-lə uyğun deyil. Reset/DB yazılması tərəfini düzəltmək lazımdır.');
